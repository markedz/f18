...
//compute signature to avoid recursive std::visit that would be expensive when comparing arg list with a function signature
static_assert(std::variant_size_v<SomeHostType> < 256, "Need bigger signature holder");
using SignatureType = unsigned char;
template<size_t N>
using SignatureArrayType = std::array<SignatureType, N>;

template<typename T>
constexpr SignatureType SignatureFromType() {
...
struct SignatureHelper {
  ...
  static constexpr auto BuildSignatureArray(std::index_sequence<i...>) {
      return SignatureArrayType<sizeof...(i)>{{SignatureFromType<std::tuple_element_t<i, std::tuple<T...>>>()...}};
  }
...
class FunctionSymbol {
  ...
  F handle;
  static constexpr SignatureArrayType<nargs+1> signature{SignatureHelper<TR, TA...>::BuildSignatureArray()};
  private:
  template<int pos, typename ...TRA>
  constexpr TR CallHelper(std::vector<SomeHostType> &args, TRA... resolvedArgs) const {
    static_assert(pos <= nargs);
    if constexpr (pos == 0) {
      return handle(std::get<std::tuple_element_t<0, std::tuple<TA...>>>(args[0]), resolvedArgs...);
    } else {
      return CallHelper<pos-1>(args, std::get<std::tuple_element_t<pos, std::tuple<TA...>>>(args[pos]), resolvedArgs...);
    }
  }
};

//This is a bit annoying, it has to be updated every time a new rte type signature is added
// won't compile if not updated
using SomeRteFunType = std::variant<
  FunctionSymbol<float, float>,
  FunctionSymbol<double, double>,
  FunctionSymbol<float, float, float>,
  FunctionSymbol<double, double, double>,
  FunctionSymbol<float, signed char, float>,
  FunctionSymbol<float, short, float>,
  FunctionSymbol<float, int, float>,
  FunctionSymbol<float , long long int, float>,
  FunctionSymbol<double, signed char, double>,
  FunctionSymbol<double, short, double>,
  FunctionSymbol<double, int, double>,
...
static const RteFunction rteFuns[]{
  ...
  {{"bessel_jn"}, {FunctionSymbol<float, signed char, float>{bessel_jn<signed char, float>}}},
  {{"bessel_jn"}, {FunctionSymbol<float, short, float>{bessel_jn<short, float>}}},
  {{"bessel_jn"}, {FunctionSymbol<float, int, float>{bessel_jn<int, float>}}},
  {{"bessel_jn"}, {FunctionSymbol<float, long long int, float>{bessel_jn<long long int, float>}}},
  {{"bessel_jn"}, {FunctionSymbol<double, signed char, double>{bessel_jn<signed char, double>}}},
  {{"bessel_jn"}, {FunctionSymbol<double, short, double>{bessel_jn<short, double>}}},
  {{"bessel_jn"}, {FunctionSymbol<double, int, double>{bessel_jn<int, double>}}},
  {{"bessel_jn"}, {FunctionSymbol<double, long long int, double>{bessel_jn<long long int, double>}}}
};

template<typename T>
class RteFunctionRef {
  public:
  using Result = T;
...
class RuntimeFunctionMap {
  ...
  std::optional<const RteFunction*> find(RteFunctionRef<T> &ref) {
    ...
    for (auto iter{rteFunRange.first}; iter != rteFunRange.second; ++iter) {
      if (nargs == iter->second->nargs && resTypeSignature == iter->second->signature[0]) {
        bool match{true};
        int pos{1};
...
std::optional<SomeHostType> ConvertToHostTypeIfConstant(const Expr<SomeType> &expr) {
 return std::visit([](const auto &x) -> std::optional<SomeHostType> {
    ...
    if constexpr (std::is_same_v<R, Expr<SomeReal>> ||
      ...
      return std::visit([](const auto &y) ->std::optional<SomeHostType>{
...
std::optional<Constant<T>> ConvertFromHostType(const HostType<T> &value) {
  if constexpr (common::HasMember<T, RealTypes> || common::HasMember<T, IntegerTypes>) {
